...
const sizeMap = {
    sm: { icon: 14, wrapper: "p-1", text: "text-lg" },
    md: { icon: 16, wrapper: "p-1", text: "text-xl" },
    lg: { icon: 18, wrapper: "p-1.5", text: "text-2xl" },
};
...
export const Logo = ({ size = "md", className = "" }: LogoProps) => {
    ...
    return (
        <div className={`z-9999 flex items-center gap-2 ${className}`.trim()}>
            ...
            <span
                className={`${currentSize.text} font-bold tracking-tight select-none ${palette.text}`}
            >
                DevPulse
            </span>
        </div>
